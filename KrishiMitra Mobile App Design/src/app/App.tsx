...
import { KvkQueriesListScreen, KvkChatThreadScreen, KvkNewQueryScreen, SubscriptionScreen, INITIAL_KVK_QUERIES } from "./components/kvk-screens";
import type { KvkQuery } from "./components/kvk-screens";
...
type Screen =
  ...
  | "kvkQueries" | "kvkChat" | "kvkNewQuery" | "subscription";
  ...
  return (
    <div className="absolute inset-0 bg-background flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-28">
        ...
        <button
          onClick={() => navigate("kvkNewQuery")}
          className="w-full py-3.5 rounded-2xl font-bold text-base flex items-center justify-center gap-2 shadow active:scale-95 transition-transform mb-3"
          style={{ background: "#FFF4E6", color: C.accent, border: `1.5px solid ${C.accent}` }}
        >
          <Headphones size={20} />
          Ask KVK Expert
        </button>
...
function ProfileScreen({ navigate, goBack }: { navigate: (s: Screen) => void; goBack: () => void }) {
  ...
  const menuItems = [
    ...
    { icon: Headphones, label: "KVK Expert Help", sub: "Ask district KVK scientists", screen: "kvkQueries" as Screen, color: "#2E7D32" },
    { icon: Crown, label: "Subscription", sub: "Upgrade for unlimited KVK queries", screen: "subscription" as Screen, color: C.accent },
        ...
        <div className="bg-card rounded-2xl p-4 mb-5" style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.07)", border: `1px solid ${C.border}` }}>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={expenseData} barGap={4}>
              ...
              />
              <Bar name="Income" dataKey="income" fill={C.primary} radius={4} />
              <Bar name="Expense" dataKey="expense" fill={C.accent + "80"} radius={4} />
            </BarChart>
          </ResponsiveContainer>
...
function AppInner() {
  ...
  // KVK state
  const [kvkFreeLeft, setKvkFreeLeft] = useState(2);
  const [kvkIsSubscribed, setKvkIsSubscribed] = useState(false);
  const [kvkQueries, setKvkQueries] = useState<KvkQuery[]>(INITIAL_KVK_QUERIES);
  const [kvkActiveQueryId, setKvkActiveQueryId] = useState("1");
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* KVK Expert Help screens */}
        {screen === "kvkQueries" && (
          <KvkQueriesListScreen
            goBack={goBack}
            freeLeft={kvkFreeLeft}
            isSubscribed={kvkIsSubscribed}
            queries={kvkQueries}
            onOpenThread={(id) => { setKvkActiveQueryId(id); navigate("kvkChat"); }}
            onNewQuery={() => {
              if (kvkFreeLeft === 0 && !kvkIsSubscribed) navigate("subscription");
              else navigate("kvkNewQuery");
            }}
        ...
        {screen === "kvkNewQuery" && (
          <KvkNewQueryScreen
            goBack={goBack}
            fromScan={history[history.length - 1] === "scanResult"}
            onSubmit={(newQuery) => {
              setKvkQueries((q) => [newQuery, ...q]);
              setKvkActiveQueryId(newQuery.id);
              setKvkFreeLeft((n) => Math.max(0, n - 1));
              navigate("kvkChat");
            }}
          />
        )}
        {screen === "subscription" && (
          <SubscriptionScreen
            goBack={goBack}
            freeUsed={3 - kvkFreeLeft}
            isSubscribed={kvkIsSubscribed}
            onSubscribeSuccess={() => {
              setKvkIsSubscribed(true);
              navigate("kvkNewQuery");
            }}
          />
        )}
